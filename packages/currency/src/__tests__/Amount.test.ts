...
describe('Amount', () => {
  ...
  describe('constructor', () => {
    it('works', () => {
      const token = new Token({ chainId: 2004, address: ADDRESS_ONE, decimals: 18 })
  ...
  describe('#ether', () => {
    it('produces ether amount', () => {
      const amount = Amount.fromRawAmount(Native.onChain(2004), 100)
      expect(amount.quotient).toEqual(JSBI.BigInt(100))
      expect(amount.currency).toEqual(Native.onChain(2004))
    })
  })

  it('token amount can be max uint256', () => {
    const amount = Amount.fromRawAmount(new Token({ chainId: 2004, address: ADDRESS_ONE, decimals: 18 }), MAX_UINT256)
    expect(amount.quotient).toEqual(MAX_UINT256)
  })
  it('token amount cannot exceed max uint256', () => {
    expect(() =>
      Amount.fromRawAmount(
        new Token({ chainId: 2004, address: ADDRESS_ONE, decimals: 18 }),
  ...
  describe('#toFixed', () => {
    it('throws for decimals > currency.decimals', () => {
      const token = new Token({ chainId: 2004, address: ADDRESS_ONE, decimals: 0 })
